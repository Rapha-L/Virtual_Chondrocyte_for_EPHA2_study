function [] = inSilicoConditions(saturation,runumber,attractor_file,init_attr,nodes,inputvalues)
...
%%
    ...
    %% if nodenames
%     for i=1:length(All_attractors)
%         All_attractors{i}=All_attractors{i}(2:4,:);
%     end
%     All_attractors = cellfun(@str2double,All_attractors,'UniformOutput',false);
    %%
    attr_init = All_attractors{init_attr};
...
% attractor{i} is a matrix of size=(3*46) where the first line is the node activity and each colone corresponds to one node
    ...
    if ~isequal(attr_init,zupdates_AC_EPHA2(per))
        [attrcnt,~,stateruns,timeseries] = ssattr(per, runs, All_attractors, fixedtime,nodes);
        twicepresult = attrcnt;
...
string = ['inSilico_condition (' num2str(nodes) ')_' num2str(runumber) '.mat'];
save(string,'saturation','twicepresult','nodes','inputvalues','All_attractors','init_attr','storeNewattr','timeseries')
   

end

function [attrcnt,newattr,stateruns,timeseries] = ssattr(initstate,runs,attractors, fixedtime, fixednodes)
...
%fixenodes=vector of nodes to be kept on fixed values
stateruns=cell(runs,1);
attrcnt = zeros(1,length(attractors));
timeseries=cell(runs,1);
...
attr = 0;  
 while (attr == 0)
    t = t + 1;
            ...
            timeseries{z}(t,:,1)= state(1,:);
            timeseries{z}(t,:,2)= state(2,:);
            timeseries{z}(t,:,3)= state(3,:);
...
%            change = 1;
      ...
      timeseries{z}(t+1,:,1)= state(1,:);
      timeseries{z}(t+1,:,2)= state(2,:);
      timeseries{z}(t+1,:,3)= state(3,:);
